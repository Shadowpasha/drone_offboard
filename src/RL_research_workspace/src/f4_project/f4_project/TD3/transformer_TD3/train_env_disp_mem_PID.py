...
class DroneGazeboEnv(gym.Env):
    def __init__(self):
        ...
        self.pid_distance =PID(0.4,0.01,0.0001,0.007,0.45,(-3.5,3.5),(-0.6,0.6))
    ...
    def step(self, action):
        ...
        if not self.done:
                time_difference = (time.time_ns() - self.prev_time)/pow(10,9)
        
                if(time_difference <= 0.0):
                    time_difference = 0.00001
                reward = self.pid_distance.update(0.0, abs(self.distance), time_difference)
                self.prev_distance = self.distance
                self.prev_time = time.time_ns()
        else:

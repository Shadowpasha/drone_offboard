...
class CrossQ_GLBATT_Actor(nn.Module):
...
class CrossQ_GLBATT_Critic(nn.Module):
...
class CrossQ_GLBATT(object):
    def __init__(self, state_dim, action_dim, max_action, discount=0.99, tau=1.0, policy_noise=0.3, noise_clip=0.6, policy_freq=3, belief_state_indices=None, td3_mode=False):
        ...
        self.belief_state_indices = belief_state_indices
        self.belief_dim = len(belief_state_indices) if belief_state_indices is not None else 4
        self.actor = CrossQ_GLBATT_Actor(state_dim, action_dim, max_action, belief_state_indices=belief_state_indices).to(device)
        ...
        self.critic = CrossQ_GLBATT_Critic(state_dim, action_dim, belief_dim=self.belief_dim, num_critics=4).to(device)

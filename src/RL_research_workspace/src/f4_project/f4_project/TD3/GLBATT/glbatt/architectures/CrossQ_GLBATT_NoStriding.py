...
class CrossQ_GLBATT_NoStriding_Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action, latent_dim=128, belief_state_indices=None):
        ...
        self.fusion_norm = nn.BatchNorm1d(self.d_model * 3)
    ...
    def forward(self, state_seq):
        ...
        # NO STRIDING: Process the entire sequence normally
        strided_seq = state_seq
        token_indices = torch.arange(0, seq_len).to(device)
...
class CrossQ_GLBATT_NoStriding_Critic(nn.Module):
...
class CrossQ_GLBATT_NoStriding(object):
    def __init__(self, state_dim, action_dim, max_action, discount=0.99, tau=1.0, policy_noise=0.3, noise_clip=0.6, policy_freq=3, belief_state_indices=None, td3_mode=False):
        ...
        self.belief_state_indices = belief_state_indices
        self.belief_dim = len(belief_state_indices) if belief_state_indices is not None else 4
        self.actor = CrossQ_GLBATT_NoStriding_Actor(state_dim, action_dim, max_action, belief_state_indices=belief_state_indices).to(device)
        ...
        self.critic = CrossQ_GLBATT_NoStriding_Critic(state_dim, action_dim, belief_dim=self.belief_dim, num_critics=4).to(device)

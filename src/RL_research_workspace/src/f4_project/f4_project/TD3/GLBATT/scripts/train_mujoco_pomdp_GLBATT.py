...
def train():
    ...
    parser.add_argument("--expl_noise", default=0.1, type=float, help="Final exploration noise")
    parser.add_argument("--expl_noise_start", default=1.0, type=float, help="Initial exploration noise")
    parser.add_argument("--expl_noise_decay_steps", default=100000, type=int, help="Steps over which to decay noise")
    ...
    file_name = f"GLBATT_POMDP_{args.env}_{args.seed}_{timestamp}"
    print(f"---------------------------------------")
    print(f"Policy: GLBATT_POMDP, Env: {args.env}, Seed: {args.seed}")
    print(f"---------------------------------------")

    if not os.path.exists("../results"): os.makedirs("../results")
    if args.save_model and not os.path.exists("../models"): os.makedirs("../models")

    # Wrap the standard environment to mask the distal joint
    ...
    # Node 0 is Hazard Horizon, remaining nodes predict masked joints
    masked_dim = 2 if "Hopper" in args.env else 2
    belief_dim = 1 + masked_dim
    belief_state_indices = list(range(belief_dim))

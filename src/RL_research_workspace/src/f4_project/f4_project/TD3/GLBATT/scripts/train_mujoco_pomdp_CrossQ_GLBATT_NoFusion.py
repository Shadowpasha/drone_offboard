...
from glbatt.architectures import CrossQ_GLBATT_NoFusion
...
def train():
    ...
    file_name = f"CrossQ_GLBATT_NoFusion_POMDP_{args.env}_{args.seed}_{timestamp}"
    if args.td3_mode: file_name += "_TD3Mode"
    print(f"---------------------------------------")
    print(f"Policy: CrossQ_GLBATT_NoFusion POMDP, Env: {args.env}, Seed: {args.seed}")
    print(f"---------------------------------------")
    ...
    policy = CrossQ_GLBATT_NoFusion.CrossQ_GLBATT_NoFusion(**kwargs)

...
def train():
    ...
    parser.add_argument("--expl_noise_start", default=1.0, type=float)
    parser.add_argument("--expl_noise_decay_steps", default=100000, type=int)
    ...
    parser.add_argument("--model", default="GLBATT", choices=["GLBATT", "NoGlobal", "NoIntention", "NoTemporal", "Context", "Summary"], help="Architecture variant for ablation studies")
    parser.add_argument("--seq_len", default=50, type=int, help="Sequence length (100 steps window, strided down to 30 in Actor)")
    parser.add_argument("--no_reward_scaling", action="store_true", default=False, help="Disable reward scaling (div by 10)")
    args = parser.parse_args()

    # Dynamic Model Selection for Ablation Studies
    if args.model == "GLBATT":
        from glbatt.architectures import GLBATT_Summary as GLBATT_Lib
    elif args.model == "NoGlobal":
        from glbatt.architectures import GLBATT_NoGlobal as GLBATT_Lib
    elif args.model == "NoIntention":
        import HCAN_NoIntention as GBLATT_Lib
    else:
        from glbatt.architectures import GLBATT_Summary as GLBATT_Lib

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    file_name = f"{args.model}_{args.env}_{args.seed}_{timestamp}"
    print(f"---------------------------------------")
    print(f"Policy: {args.model}, Env: {args.env}, Seed: {args.seed}")
    print(f"---------------------------------------")
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.expl_noise_decay_steps:
            current_expl_noise = args.expl_noise_start - (args.expl_noise_start - args.expl_noise) * (t / args.expl_noise_decay_steps)
        else:
            current_expl_noise = args.expl_noise

        if t < args.start_timesteps:
            action = env.action_space.sample()
        else:
            action = (policy.select_action(state_seq) + np.random.normal(0, max_action * current_expl_noise, size=action_dim)).clip(-max_action, max_action)
        ...
        if t >= args.start_timesteps:
            ...
            if (t + 1) % 100 == 0:
                ...
                writer.add_scalar("Stats/ExplorationNoise", current_expl_noise, t + 1)

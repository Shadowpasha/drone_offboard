...
def train():
    ...
    parser.add_argument("--tau", default=1.0, type=float) # CrossQ default
    parser.add_argument("--policy_noise", default=0.2, type=float)
    parser.add_argument("--noise_clip", default=0.5, type=float)
    parser.add_argument("--policy_freq", default=3, type=int) # CrossQ default
    ...
    file_name = f"CrossQ_GLBATT_Gym_{args.env}_{args.seed}_{timestamp}"
    print(f"---------------------------------------")
    print(f"Policy: CrossQ_GLBATT_Gym, Env: {args.env}, Seed: {args.seed}, TD3 Mode: {args.td3_mode}")
    print(f"---------------------------------------")
    ...
    # Dynamic target mapping based on environment (copied from train_gym_GLBATT.py)
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t >= args.start_timesteps:
            # CrossQ_GLBATT.train returns: Q_value, critic_loss, z_var, final_attn, intrinsic_avg, safety_acc
            ...
            if (t + 1) % 100 == 0:
                ...
                # Attn weights are for 30 tokens (10 global, 20 local)

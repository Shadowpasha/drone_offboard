...
class Holonomic360LidarEnv(HolonomicLidarEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        # NOTE: Keep in sync with the ±4.0 clipping bounds in step()
        self.world_size = 8.0
    ...
    def _get_obs(self):
        ...
        max_dist = self.world_size * 1.414  # max diagonal of the world
        state_info = np.array([
            self.last_action[0],
            self.last_action[1],
            dist / max_dist,  # Normalized by actual world diagonal
            heading_diff / np.pi,
    ...
    def step(self, action):
        ...
        if goal_reached:
            ...
            # Cache raycast result to avoid computing it twice per step
            lidar_readings = self._raycast()
            ...
            # Activation at 0.8m (tighter), max penalty -0.05 (well below progress signal)
            min_clearance = np.min(lidar_readings) * self.lidar_max_range
            if min_clearance < 0.8:
                reward -= 0.05 * ((0.8 - min_clearance) / 0.8) ** 2    
